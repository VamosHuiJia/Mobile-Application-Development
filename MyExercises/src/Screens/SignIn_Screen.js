...
const SignIn = ({ navigation, route }) => {
  const { setIsLoggedIn } = useContext(AppContext); // Lấy hàm cập nhật trạng thái đăng nhập
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const isLogin = route.name === "Login";

  // Kiểm tra nếu email hoặc password trống thì disable nút đăng nhập
  const isButtonDisabled = !email.trim() || !password.trim();

  const handleAuth = () => {
    if (isLogin) {
      console.log("Đăng nhập với", email, password);
      setIsLoggedIn(true); // Chuyển hướng đến Home
    } else {
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Nút đăng nhập bị vô hiệu hóa nếu email hoặc password trống */}
      <TouchableOpacity 
...
const styles = StyleSheet.create({
  ...
  disabledButton: { backgroundColor: "gray" }, // Màu xám khi nút bị vô hiệu hóa
